...
Title = tkinter.Label(root, text='融媒体违法广告识别软件',fg='black',font=("华文新魏",25), justify=tkinter.RIGHT, anchor='e', width=200)
# 显示该组件的位置及大小
...
labeName = tkinter.Label(root, text='用户名：',font=("华文新魏",16), justify=tkinter.RIGHT, anchor='e', width=80)
# 显示该组件的位置及大小
...
# 在窗口上创建标签组件（User Pwd）
labeName = tkinter.Label(root, text='密码：',font=("华文新魏",16), justify=tkinter.RIGHT, anchor='e', width=80)
# 显示该组件的位置及大小
...
checkRemember = tkinter.Checkbutton(root, text='记住密码',font=("华文新魏",14), variable=rememberMe, onvalue=1, offvalue=0)
...
def user_login():
    ...
    conn = {}    
    if db_sqlite:
        conn = sqlite3.connect("sqlite.db")     #建立一个基于硬盘的数据库实例
    else:    
        conn = pymysql.connect(host="localhost", user="root", password="123456", database="login_sql", charset="utf8")
    curs = conn.cursor()
    # 执行SQL语句，创建user数据表
    ...
    # 账号不在数据库中，则弹出是否注册的框
        ...
        is_signup = tkinter.messagebox.askyesno(title='提示', message='该账号不存在，是否现在注册？')
        if is_signup:
            user_register()
...
buttonOk = tkinter.Button(root, text='登录', activeforeground='#ff0000', command=user_login)
buttonOk.place(x=170, y=215, width=80, height=25)

buttonRegister = tkinter.Button(root, text='注册', activeforeground='#ff0000', command=user_register)
